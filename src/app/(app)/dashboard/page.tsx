...
export default async function DashboardPage() {
  ...
  const salairesPrecis =
    (
      companyWithEmployees.employees ??
      []
    ).some(
      (employee) =>
        Number(
          employee.salaireBrutMensuel,
        ) > 0,
    )
  ...
  const deadlineData =
    deadline
      ? {
          obligation:
            `${deadline.type.toUpperCase()} — ${deadline.title}`,

          date:
            deadline.eventDate.toLocaleDateString(
              'fr-FR',
              {
                day: 'numeric',
                month: 'long',
          ...
          montant:
            deadline.montantEstime != null
              ? fmtFCFA(
                  Number(
                    deadline.montantEstime,
                  ),
                )
              : 'À calculer',
  ...
  const repartitionFiscale =
    applicableTaxes.map(
      (tax) => {
        const montant =
          montantsParCategorie.get(
            tax.key,
          )
  ...
  const transactions =
    recentTx.map(
      (tx) => {
        const contribuable =
          tx.declaration
            ? `${tx.declaration.type.toUpperCase()} — ${tx.declaration.periode}`
  ...
  const totalPayeValue =
    Number(
      totalPaye._sum.amount ??
        0,
    )
